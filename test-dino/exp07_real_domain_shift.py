...
сработает на них, а не на породе:
  ...
  2. Масштаб. У Сколтеха ~1.2 мкм/воксель (Ачимовская свита, Ebadi et al.),
     у нас 1.84 мкм/воксель. Разница в 1.53 раза: без поправки "другая порода"
     смешается с "другое увеличение".
...
Три варианта подачи своих данных, каждый отвечает на свой вопрос:
  ...
  rescaled      — окно своё + пересчёт 1.84 -> 1.2 мкм/воксель.
...
VOX_OWN, VOX_SK = 1.84, 1.2    # мкм на воксель
...
def load_own_frames(rescale: bool) -> np.ndarray:
    ...
    rescale=True — сначала берём область в 1.53 раза больше и сжимаем,
    чтобы поры имели такой же размер в пикселях, как у Сколтеха."""
...
def main():
    ...
    sk_ref, sk_ctrl = sk_all[:N_REF], sk_all[N_REF:]
